...
module SpaceGame exposing (main)
...
import Sphere3d
import Speed
import SpaceshipPart exposing (Action(..), Key(..), KeyAction(..), Spaceship, customPart, initSpaceshipWorld, merlin, movePart, rotatePart)
import Task
import Temperature
...
earthSize = (Length.inCentimeters <| Length.kilometers 6371)
satelliteSize = (Length.inCentimeters <| Length.kilometers 600)
    ...
    in
        [   
          sphere earthSize
            ...
            -- |> withMass 1000
          ,  
          satellite model model.ifSatelliteBroken
            |> rotateY3D (degrees 180)
            |> scaleTo3D satelliteSize Direction3d.y
            |> move3D (0, earthSize*2, earthSize)
            |> usePhysics "satellite"
            --|> withMass 1000 
        ]
...
myPhysicsUpdates model =
    [ 
        updateIf 
          (\body -> (Body.data body).name == "earth")
          (rotateBody (Angle.degrees 0) (Angle.degrees 5) (Angle.degrees 0))
    , updateIf 
          (\body -> (Body.data body).name == "satellite")
          (rotateBody (Angle.degrees 0) (Angle.degrees 0.5) (Angle.degrees 0))
...
physicsDebug objToTrack model =
    ...
    in
    group 
        [ text ("Name: " ++ trackingName) |> filled black |> move (-300,50)
        , text ("Velocity: " ++ Debug.toString (Maybe.map Body.velocity <| List.head <| toTrack)) |> filled black |> move(-300,35)
        , text ("Origin Point: " ++ Debug.toString (Maybe.map (Frame3d.originPoint << Body.frame) <| List.head <| toTrack)) |> filled black |> move(-300,20)
        , text ("Rotational velocity: " ++ Debug.toString (Maybe.map Body.angularVelocity <| List.head <| toTrack)) |> filled black |> move(-300,5)
        , text ("x direction: " ++ Debug.toString (Maybe.map (Frame3d.xDirection << Body.frame) <| List.head <| toTrack)) |> filled black |> move(-300,-10)
        , text ("y direction: " ++ Debug.toString (Maybe.map (Frame3d.yDirection << Body.frame) <| List.head <| toTrack)) |> filled black |> move(-300,-25)
        , text ("z direction: " ++ Debug.toString (Maybe.map (Frame3d.zDirection << Body.frame) <| List.head <| toTrack)) |> filled black |> move(-300,-40)
        ] |> scale 2 |> move (0, 250)
...
type Msg
    ...
    | AddBodies
    -- TODO maybe add a "HitBody" message that takes an impulse force and name and applies it to that body?
    ...
    in
        Html.div []
            ...
            ,   Scene3d.custom
                    ...
                    , entities =  baseEntities ++ renderEntities (renderBodies False model.world)
                    }
    ...
    case message of
        ...
        Tick t ->
           let
            ...
            in
                ( { model | time = timeAsNum, world = updatedWorld }
                , Cmd.batch
                    [ Task.perform 
