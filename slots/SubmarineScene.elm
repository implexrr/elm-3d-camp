module SubmarineScene exposing (main)
...
{-header<b>3D Submarine Slot!</b>: Underwater physics! -}
...
myPhysicsObjects model =
    [ -- This cylinder is here as a prop for the collision example below.
        cylinder 50 100
        |> matte Color.red
        |> rotate3D (degrees 45) (degrees 45) 0
        |> move3D (200, 200, 0)
        |> usePhysics "sphere"
        |> withMassU (Mass.metricTons 10)
    ]
...
subStartingPos = (Length.meters 0, Length.meters 0, Length.meters 0)
...
collisionExample model =
    case World.contacts model.world of
        [] ->
            group []
        _ ->
            text "Something is colliding!"
                |> centered
                |> size 16
                |> sansserif
                |> filled white
...
skyboxType =
    Skybox.URLSkybox textureBottom textureTop textureSide1 textureSide2 textureSide3 textureSide4 (Length.inCentimeters skyboxSize)
...
submarineBody model =
    submarineEntity model
        |> usePhysics "submarine"
        |> withMassU (Mass.metricTons 7.5)
        |> customDamping 0.5 0.5
        |> startAtPos subStartingPos

-- Strength of the submarine's engines
forwardForce = Force.kilonewtons 1

verticalForce = Force.kilonewtons 0.25

rotationalForce = Force.kilonewtons 0.25

skyboxSize = Length.meters 100

-- This colour is used to create the floor. If you want custom colours, use Color.hsl or Color.rgb!
floorColour = Color.lightYellow

-- Create a quad to act as a 'floor'
floor =
    quad3D (Length.meters 100) (Length.meters 100)
    |> matte floorColour
    |> move3D (0,0,-100)
    |> usePhysics "floor"
...
type alias Model =
    ...
    , dirLR : Direction -- Roll
    , dirFB : Direction -- Forwards/backwards
    , dirUD : Direction -- Up/down
    , rotLR : Direction -- Left/right turning
    ...
    ( { width = Quantity.zero
      ...
      , cameraModel =
          W3C.customCamera
            ...
            (Length.meters 3, Length.meters 1, Length.meters 5)
            (Angle.degrees 45)
    ...
    case message of
        ...
        Tick t ->
            let
                ...
                submarineUpdates =
                    [ pushIf "submarine" -- Forwards and backwards motion
                        ( \ _ -> model.dirFB /= None )
                        ( case model.dirFB of
                            Forward ->
                                forwardForce
                ...
                updatedWorld =
                    List.foldl
                        ...
                        simulatedWorld
                        (myPhysicsUpdates model ++ submarineUpdates)
                    |> World.update 
                        ( \ body ->
                            let
                                data = Body.data body
            ...
            in
                ...
                , Cmd.batch
                    ...
                    , Task.perform
                        ( \ _ ->
                            Maybe.withDefault (Point3d.centimeters 0 0 20)
                                (Maybe.map
                                    (\ body ->
                                        Body.frame body
                                            |>Frame3d.placeIn Frame3d.atOrigin
                                            |> Frame3d.originPoint
                                    )
                                    (getBodyByName "submarine" model.world)
                                )
                            |> W3C.SetPos
                            |> CameraMsg
                        )
                ...
                Loaded ->
                    ...
                    | world = 
                            List.foldl 
                                ...
                                model.world (myPhysicsObjects model ++ [ floor, submarineBody model ] )
                    }
                    , Cmd.none
                    )
                Loading ->
