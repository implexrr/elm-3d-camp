module Wrapper3D exposing
    ( metallic, plastic, matte, customMat, textured, constantTexture, constantColourTexture
    , cube, square3D, rectangle3D, box
    , cubeU, boxU, quad3D, sphereU, coneU, cylinderU
    , sphere, cone, cylinder, cylinderStartingAt, arrowStartingAt, texturedCylinder
    , polyCone, polyCylinder, truncatedCone
    , ring, ellipsoid, polygon3D, customPolygon, customObject
    , move3D, move3DU, scale3D, scaleTo3D, rotate3D, rotate3DU
    , rotateX3D, rotateY3D, rotateZ3D
    , group3D
    , repeatDuration, repeatDistance, animationPieces
    , getMeshName
    , generateEllipsoid, generatePolyCone, generatePolyCylinder, generateRing, generateTexturedCylinder, generateTruncatedCone, generateDynamicMesh
    , Dimension, Mold, GeneratedMesh, MeshStore, withOverlay, unwrapQ
    , Object(..), Collider(..), CustomMesh(..), ShapeApproximation(..)
    , renderEntities, renderCollider
    , boxOutline
    , relativeP, placeObject
    , getVolume, getPosition, getCentre, getCollider, pointLight
    )
...
type alias PointList = List (Float, Float)
...
type alias Mold coordinates a = (Material coordinates { a | normals : () } -> Object coordinates)

type alias TexturedMold coordinates a = (Material coordinates { a | normals : (), uvs : () } -> Object coordinates)
...
square3D length material =
    let
        lenVal = Length.centimeters length
    in
...
quad3D length width material =
    let
        lValue = length |> Quantity.divideBy 2
        wValue = width |> Quantity.divideBy 2
        rectangleShape =
...
sphereU r material =
    let
        sphereShape = Sphere3d.withRadius r Point3d.origin
    in
        Object
...
polyCone points (xtip,ytip,ztip) model material =
    let
        meshName = getMeshName (PolyCone points (xtip,ytip,ztip))
        ...
        minX = List.minimum (List.map (\(x, _) -> x) points)
        maxX = List.maximum (List.map (\(x, _) -> x) points)
        minY = List.minimum (List.map (\(_, y) -> y) points)
        maxY = List.maximum (List.map (\(_, y) -> y) points)

        avgRadius = (((Maybe.withDefault 0 maxX - Maybe.withDefault 0 minX) |> abs) + ((Maybe.withDefault 0 maxY - Maybe.withDefault 0 minY) |> abs)) / 2
...
truncatedCone topR botR height model material =
    let
        meshName = getMeshName (TruncatedCone topR botR height)

        mesh =
            case Dict.get meshName model.generatedMeshes of
                Nothing -> truncatedConeMesh topR botR height
                Just actualMesh -> actualMesh

        shadow =
            case Dict.get meshName model.generatedShadows of
                Nothing -> Mesh.shadow (truncatedConeMesh topR botR height)
                Just actualMesh -> actualMesh
        ...
        avgRadius = (topR + botR) / 2

        boundingBox = BoundingBox3d.fromExtrema
...
polyCylinder points height model material  =
    let
        meshName = getMeshName (PolyCylinder points height)
...
cylinderStartingAt pt dir properties material =
    let
      cylinderShape = Cylinder3d.startingAt pt dir properties
    in Object
    { shape = Scene3d.cylinderWithShadow material cylinderShape
    , boundingBox = Cylinder3d.boundingBox cylinderShape |> Box
    , name = "cylinder"
...
coneStartingAt pt dir properties material =
    let
      coneShape = Cone3d.startingAt pt dir properties
    in Object
    { shape = Scene3d.coneWithShadow material coneShape
    , boundingBox = Cone3d.boundingBox coneShape |> Box
    , name = "cylinder"
...
texturedCylinder radius height model material =
    let
        meshName = getMeshName (TexturedCylinder radius height)

        mesh =
            case Dict.get meshName model.generatedMeshes of
                Nothing -> texturedCylinderMesh radius height
                Just actualMesh -> actualMesh

        shadow =
            case Dict.get meshName model.generatedShadows of
                Nothing -> Mesh.shadow (texturedCylinderMesh radius height)
                Just actualMesh -> actualMesh
...
ring radius thickness model material =
    let
        meshName = getMeshName (Ring radius thickness)

        mesh =
            case Dict.get meshName model.generatedMeshes of
                Nothing -> ringMesh radius thickness
                Just actualMesh -> actualMesh

        shadow =
            case Dict.get meshName model.generatedShadows of
                Nothing -> Mesh.shadow (ringMesh radius thickness)
                Just actualMesh -> actualMesh
...
ellipsoid length width height model material =
    let
        meshName = getMeshName (Ellipsoid (length, width, height))
...
customPolygon points material =
    let
        ...
        length = (((Maybe.withDefault 0 maxX + Maybe.withDefault 0 minX) |> abs) + ((Maybe.withDefault 0 maxY + Maybe.withDefault 0 minY) |> abs)) / 2
      ...
      case ( minX, maxX ) of
        ( Just minx, Just maxx ) ->
           case (minY, maxY) of
             (Just miny, Just maxy) ->
               Object {
                shape = Scene3d.meshWithShadow material 
                          (mesh maxx minx maxy miny) (shadow maxx minx maxy miny)
                , boundingBox = BoundingBox3d.from
...
customObject triangularMesh doCulling model material =
    let
        meshName = getMeshName (CustomObject triangularMesh doCulling)
        mesh =
            case Dict.get meshName model.generatedMeshes of
                Nothing ->
...
move3DU pt object =
    let
        tup = Point3d.toTuple Length.inCentimeters pt
    in
    move3D tup object
...
move3D (x,y,z) object =
    let
        displacement = Vector3d.centimeters x y z
    in
    case object of
        ...
        case object of
            ObjectGroup attributes ->
                ObjectGroup
                    ...
                    , boundingBox = case attributes.boundingBox of
...
placeObject : Frame3d.Frame3d Meters coordinates { defines : localCoordinates }
            ...
            Object
                ...
                , customMesh =
                        case attributes.customMesh of
                            Primitive -> Primitive
                            Ellipsoid dimensions -> Ellipsoid dimensions
                            Ring r t -> Ring r t
                            PolyCone points tipPos -> PolyCone points tipPos
                            PolyCylinder points h -> PolyCylinder points h
                            TexturedCylinder r h -> TexturedCylinder r h
                            TruncatedCone rt rb h -> TruncatedCone rt rb h
...
scaleTo3D scaleTo direction object =
  let
    boxSize box_attri = 
      let
        boxExtrema = BoundingBox3d.extrema box_attri
      in
        if direction == Direction3d.x then
          Length.inCentimeters <| Length.meters 
    ...
    let
        ...
        objListHelper entityList =
            List.foldr
                ( \ entity otherEntity ->
                    case entity of
                        ObjectGroup attributes -> objListHelper attributes.subObjects ++ otherEntity
                        Object attributes -> attributes :: otherEntity
                )
                []
                entityList
...
ringMesh radius thickness =
    let
        ...
        pointList = List.map
                    ( \idx ->
                      let
                        ratioSlow = 2 * pi / numPoints
                        ratioFast = 200 * pi / numPoints
                        t = toFloat idx
                        slowx = sin (ratioSlow*t)
                        slowy = cos (ratioSlow*t)
                        fastx = radius + thickness * sin (ratioFast *t)
                        fastz =          thickness * cos (ratioFast *t)
                      in
                        { position = 
                            Point3d.centimeters
                                (slowx * fastx)
        ...
        triangularMesh =
            TriangularMesh.indexed
                ...
                ((List.concatMap ( \ idx ->  [(idx+1,idx,16+idx)
                                            ,(idx+1,16+idx,16+idx+1)]
                                )
                                <| List.range 0 (numPoints - 16))
                ++
                (List.concatMap ( \ idx ->  [(idx + numPoints - 16 + 1, idx + numPoints - 16, 16 + idx)
...
polyCylinderMesh points height =
    let
        ...
        top = List.map
            ...
            points
        minX = Maybe.withDefault 0 (List.minimum (List.map (\(x, _) -> x) points))
        maxX = Maybe.withDefault 0 (List.maximum (List.map (\(x, _) -> x) points))
        minY = Maybe.withDefault 0 (List.minimum (List.map (\(_, y) -> y) points))
        maxY = Maybe.withDefault 0 (List.maximum (List.map (\(_, y) -> y) points))
        uWidth = maxX - minX
        vWidth = maxY - minY
        bottomOrigin = { position = Point3d.centimeters ((maxX + minX) /2) ((maxY + minY) /2) 0, uv = (0.5,0.5) }
        bottomFace = TriangularMesh.fan bottomOrigin (List.reverse bottom)
        topOrigin = { position = Point3d.centimeters ((maxX + minX) /2) ((maxY + minY) /2) height, uv = (0.5,0.5) }
        topFace = TriangularMesh.fan topOrigin top
        sides = TriangularMesh.strip bottom top
...
polyConeMesh points (xtip,ytip,ztip) =
    let
        ...
        apron = List.map
            ...
            points
        pointsIncludingTip = (xtip, ytip) :: points -- Used for UV mapping, so only 2D coordinates are required
        minX = Maybe.withDefault 0 (List.minimum (List.map (\(x, _) -> x) pointsIncludingTip))
        maxX = Maybe.withDefault 0 (List.maximum (List.map (\(x, _) -> x) pointsIncludingTip))
        minY = Maybe.withDefault 0 (List.minimum (List.map (\(_, y) -> y) pointsIncludingTip))
        maxY = Maybe.withDefault 0 (List.maximum (List.map (\(_, y) -> y) pointsIncludingTip))
        uWidth = maxX - minX
        vWidth = maxY - minY
        bottomOrigin = { position = Point3d.centimeters ((maxX + minX) /2) ((maxY + minY) /2) 0, uv = (0.5,0.5) }
        bottomFace = TriangularMesh.fan bottomOrigin (List.reverse apron)
    in
...
polygonMesh sides =
    let
        ...
        getPos n = Point3d.translateBy (Vector3d.withLength (Length.centimeters 1) (Direction3d.xy (Angle.degrees (angleDegrees * n)))) Point3d.origin

        getRange values = (Maybe.withDefault 0 (List.maximum values), Maybe.withDefault 0 (List.minimum values))

        xCoords = List.map ( \ point -> Length.inMeters (Point3d.xCoordinate point) ) (List.map (\n -> getPos (toFloat n)) (List.range 0 (actualSides - 1)))
        ...
        wedge startIndex =
            let
                ...
                uEnd =
                    ...
                    if (Angle.inRadians endAngle |> abs) < 1e-6 then
                        1
                    else
...
truncatedConeMesh topR botR height =
    let
        ...
        face = TriangularMesh.fan topOrigin topFacePoints

        sides = TriangularMesh.strip bottomPoints topPoints
            ...
            case ks of
                ...
                k :: rest ->
                    let
                        ...
                        x = s - toFloat (floor s)
                        hk = floor (2 ^ 16 * x)
                        segment = getUnicodeSegment hk (2 ^ 12) ""
                    in
